...
exports.addFeedback = [async (req, res) => {
    try {
        ...
        let update_rating_to = parseFloat((present_product_average_rating + product_rating) / 2);
...
exports.readOwnFeedback = [async (req, res) => {
    try {
...
exports.addToCart = [async (req, res) => {
    ...
    dbquery.insertSingle("cart_details", params);
    response["status"] = "success";
    response["mssg"] = "";
    response["data"] = {};
    response["data"]["countProductInCart"] = countProductInCart;
    return res.send(response);
}]

exports.countProductInCart = [async (req, res) => {
    let customer_id = req.locals.customer_id;
    let details = await dbquery.count_product_in_cart(customer_id);
    let response = {};
    response["status"] = "success";
    response["mssg"] = "";
    response["data"] = {};
...
exports.readCart=[async(req,res)=>{
    let customer_id=req.locals.customer_id;
    let details = await dbquery.get_all_cart(customer_id);
    let response={};
    if(utility.checkEmpty(details))
    {
        response["status"]="error";
        response["mssg"]="nothing in cart, keep shoping"; 
        return res.send(response);  
    }
    response["status"]="success";
    response["mssg"]="";
...
exports.removeFromCart = [async (req, res) => {
    ...
    let product_id = input.product_id;
    let customer_id = req.locals.customer_id;
    dbquery.remove_product_from_cart(customer_id, product_id);
    let response = {};
    response["status"] = "success";
    response["mssg"] = "cart removed successfully";
    return res.send(response);
}]
exports.readOthersFeedback = [async (req, res) => {
    let input = req.body.inputData;
    let max_limit = input.max_limit;
    let response = {};
    let customer_id = req.locals.customer_id;
    if (utility.checkEmpty(customer_id)) {
        let details = await dbquery.read_others_feedback(max_limit);
        response["status"] = "success";
        response["mssg"] = "";
        response["data"] = details;
        return res.send(response);
    }
    let details = await dbquery.read_feedback_excluding_own_to_limits(customer_id, max_limit);
